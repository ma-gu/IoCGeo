using BCJobs.Analytics.Geocoding.Convertes;
...
{
    [TypeConverter(typeof(MapPolygonTypeConverter))]
    [JsonConverter(typeof(MapPolygonJsonConverter))]
    public struct MapPolygon : IEnumerable<MapPoint>, IEquatable<MapPolygon>
    {
        public static readonly MapPolygon? None = null;
        ...
        public MapPoint SouthWest => new MapPoint(
            Points.Select(p => p.Latitude).Min(),
            Points.Select(p => p.Longitude).Max());
        public MapPoint NorthEast => new MapPoint(
